"""
Run forecasting for kShape clustering only
"""
...
clusters_df = pd.read_csv('results/clusters/kshape_clusters.csv')
cluster_labels = clusters_df['cluster'].values.astype(int)
n_clusters = int(np.max(cluster_labels)) + 1

print(f"kShape - Number of clusters: {n_clusters}")
...
try:
    ...
    val_smape = np.mean(results['val_SMAPE'])
    test_smape = np.mean(results['test_SMAPE'])
