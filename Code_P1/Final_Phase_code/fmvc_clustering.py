"""
FMVC clustering on interpolated + Z-normalized data (5000 customers)
Uses FIXED/uniform view weights (learn_weights=False) — this is what
distinguishes plain FMVC from OFMVC (which learns optimal weights).
"""
...
print("\nRunning FMVC (fixed/uniform view weights, NOT learned)...")
Final, used_weights = main_clustering(
    viewpoint=viewpoint,
    ...
    learn_weights=False   # <-- explicit: FMVC keeps weights fixed/uniform
)

print("\n" + "="*60)
print("VIEW WEIGHTS USED (should all be ~equal — confirms FMVC, not OFMVC)")
print("="*60)
for name, weight in zip(view_names, used_weights):
...
result_df.to_csv('results/clusters/fmvc_clusters_final.csv', index=False)

print("\n FMVC clustering completed.")
